...
func TestParityAgainstI2SVectors(t *testing.T) {
	...
	rtol := envFloat32("BITNET_I2S_LOGIT_RTOL", 1e-1)
...
func TestParityAgainstI2SSmoke(t *testing.T) {
	if os.Getenv("BITNET_ENFORCE_I2S_SMOKE") != "1" {
		t.Skip("set BITNET_ENFORCE_I2S_SMOKE=1 to run i2_s smoke parity")
	}
	...
	modelPath := filepath.Join(root, string(bytesTrimSpace(modelFixture)))
	if _, err := os.Stat(modelPath); err != nil {
		t.Skipf("i2_s model not present: %s", modelPath)
	}
...
func maybeForceTokens(t *testing.T, want []int32) {
	t.Helper()
	if os.Getenv("BITNET_PARITY_FORCE_TOKENS") != "" {
		return
	}
	if os.Getenv("BITNET_PARITY_FORCE") != "1" {
		return
	}
	var b strings.Builder
	for i, tok := range want {
		...
		b.WriteString(strconv.FormatInt(int64(tok), 10))
	}
	t.Setenv("BITNET_PARITY_FORCE_TOKENS", b.String())
}
