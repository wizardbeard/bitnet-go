...
FAMILY=${1:-i2s}
STEP=${BITNET_KF32_SWEEP_STEP:-14}
TOKEN=${BITNET_KF32_SWEEP_TOKEN:-55358}
ROOT_DIR=$(CDPATH= cd -- "$(dirname -- "$0")/.." && pwd)
OUT_BASE=${BITNET_KF32_SWEEP_OUT_BASE:-.bench/kf32-layermax-sweep}
SUMMARY=${BITNET_KF32_SWEEP_SUMMARY:-.bench/kf32-layermax-sweep-${FAMILY}.tsv}
LAYER_MAX_LIST=${BITNET_KF32_SWEEP_LAYERS:-0 4 8 12 14}
...

{
  ...
  run_case "kf32_all_layers"
  for layer_max in $LAYER_MAX_LIST; do
    run_case "kf32_l${layer_max}" BITNET_STRICT_K_F32_LAYER_MAX="$layer_max"
  done
} > "$SUMMARY"

echo "kf32 layer-max sweep summary: $SUMMARY"
cat "$SUMMARY"
