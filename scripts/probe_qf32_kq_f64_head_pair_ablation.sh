...
FAMILY=${1:-i2s}
Q_LAYER_MAX=${BITNET_QF32_KQ_F64_PAIR_Q_LAYER_MAX:-6}
KQ_LAYER_MAX=${BITNET_QF32_KQ_F64_PAIR_KQ_LAYER_MAX:-12}
STEP=${BITNET_QF32_KQ_F64_PAIR_STEP:-2}
TOKEN=${BITNET_QF32_KQ_F64_PAIR_TOKEN:-40}
ROOT_DIR=$(CDPATH= cd -- "$(dirname -- "$0")/.." && pwd)
OUT_DIR=${BITNET_QF32_KQ_F64_PAIR_OUT_DIR:-.bench/qf32-kq-f64-head-pair-ablation}
SUMMARY=${BITNET_QF32_KQ_F64_PAIR_SUMMARY:-$OUT_DIR/${FAMILY}.tsv}
...
run_case() {
  ...
  printf "%s\t%s\t%d\t%s\t%s\t%s\t%s\t%s\t%s\n" \
    "$name" "$q_heads_csv" "$status" \
    "${mismatch_step:-}" "${mismatch_token:-}" "${got:-}" "${want:-}" "${abs_err:-}" "${step_logit:-}"
}

{
  printf "case\tq_heads\ttest_status\tmismatch_step\tmismatch_token\tmismatch_got\tmismatch_want\tmismatch_abs_err\tstep_logit\n"
...
echo "qf32+kq f64 head pair ablation summary: $SUMMARY"
cat "$SUMMARY"
