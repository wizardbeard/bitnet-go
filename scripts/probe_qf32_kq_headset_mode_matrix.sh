...
FAMILY=${1:-i2s}
Q_LAYER_MAX=${BITNET_QF32_KQ_HEADSET_Q_LAYER_MAX:-6}
KQ_LAYER_MAX=${BITNET_QF32_KQ_HEADSET_KQ_LAYER_MAX:-12}
STEP=${BITNET_QF32_KQ_HEADSET_STEP:-2}
TOKEN=${BITNET_QF32_KQ_HEADSET_TOKEN:-40}
MODES=${BITNET_QF32_KQ_HEADSET_MODES:-ggml naive f64}
ROOT_DIR=$(CDPATH= cd -- "$(dirname -- "$0")/.." && pwd)
OUT_DIR=${BITNET_QF32_KQ_HEADSET_OUT_DIR:-.bench/qf32-kq-headset-mode-matrix}
SUMMARY=${BITNET_QF32_KQ_HEADSET_SUMMARY:-$OUT_DIR/${FAMILY}.tsv}
...
run_case() {
  ...
  name=$2
  q_heads_csv=$3
  log="$OUT_DIR/${FAMILY}-${mode}-${name}.log"
  set +e
  env \
    "$ENFORCE_NAME=1" \
    ...
    BITNET_STRICT_Q_F32_HEADS="$q_heads_csv" \
    BITNET_STRICT_KQ=1 \
  ...
  printf "%s\t%s\t%s\t%s\t%d\t%s\t%s\t%s\t%s\t%s\t%s\n" \
    "$mode" "$name" "$q_heads_csv" "$log" "$status" \
    "${mismatch_step:-}" "${mismatch_token:-}" "${got:-}" "${want:-}" "${abs_err:-}" "${step_logit:-}"
}

{
  printf "kq_mode\tcase\tq_heads\tlog_path\ttest_status\tmismatch_step\tmismatch_token\tmismatch_got\tmismatch_want\tmismatch_abs_err\tstep_logit\n"
  for mode in $MODES; do
    run_case "$mode" "all_q_heads" ""
    run_case "$mode" "q_head0_only" "0"
    run_case "$mode" "q_head2_only" "2"
    run_case "$mode" "q_head0_2" "0,2"
    run_case "$mode" "q_head0_1" "0,1"
    run_case "$mode" "q_head2_3" "2,3"
  done
} > "$SUMMARY"

echo "qf32+kq headset/mode matrix summary: $SUMMARY"
cat "$SUMMARY"
