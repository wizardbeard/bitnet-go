...
FAMILY=${1:-i2s}
STEP=${BITNET_QF32_SWEEP_STEP:-14}
TOKEN=${BITNET_QF32_SWEEP_TOKEN:-55358}
ROOT_DIR=$(CDPATH= cd -- "$(dirname -- "$0")/.." && pwd)
OUT_BASE=${BITNET_QF32_SWEEP_OUT_BASE:-.bench/qf32-layermax-sweep}
SUMMARY=${BITNET_QF32_SWEEP_SUMMARY:-.bench/qf32-layermax-sweep-${FAMILY}.tsv}
LAYER_MAX_LIST=${BITNET_QF32_SWEEP_LAYERS:-0 4 8 12 14}
...

{
  ...
  run_case "qf32_all_layers"
  for layer_max in $LAYER_MAX_LIST; do
    run_case "qf32_l${layer_max}" BITNET_STRICT_Q_F32_LAYER_MAX="$layer_max"
  done
} > "$SUMMARY"

echo "qf32 layer-max sweep summary: $SUMMARY"
cat "$SUMMARY"
