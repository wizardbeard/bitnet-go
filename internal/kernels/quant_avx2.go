...
func init() {
	...
	if os.Getenv("BITNET_FORCE_AVX2") == "1" || C.bitnet_has_avx2() != 0 {
		...
		matVecI2SI8SFastRange = matVecI2SI8SAVX2Range
...
func matVecI2SI8SAVX2Range(dst []float32, packed []byte, rows, cols int, vec []int8, weightScale, actScale float32, actSum int32, cStart, cEnd int) bool {
	...
	// The AVX2 kernel expects packed matrix data to begin at a column-aligned offset.
	if rows%128 != 0 {
		return false
	}
	...
	C.matvec_i2s_i8s_avx2(
		...
		C.int(rows),
		C.int(chunkCols),
		(*C.schar)(unsafe.Pointer(&vec[cStart])),
		C.float(weightScale),
		C.float(actScale),
		C.int(actSum),
	)
